	...
	protected NextAction create(Scanner sc) {
		...
		options.add(
			new MethodMenuOption(
				"Aceitar",
				(scanner) -> validateAndSaveNew(
					dummySupplier,
					(s) -> SupplierValidations.validateAll(s),
					(s) -> EcommerceData.supplierRepository.save(s))));
		
		new Menu("Novo fornecedor", options)
			.setDetailsToShow(dummySupplier)
		...
		options.add(
				new MethodMenuOption(
					...
					(scanner) -> validateAndSaveNew(
						selectedSupplier,
						(s) -> SupplierValidations.validateAll(s),
						(s) -> EcommerceData.supplierRepository.update(s))));
		
		new Menu("Alterar o fornecedor " + selectedSupplier.getId(), options)
